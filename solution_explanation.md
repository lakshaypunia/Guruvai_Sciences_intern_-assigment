...
**`can_attend_all` & `min_rooms_required`:** For both, the time complexity is $O(n \log n)$ as both functions include sorting whose time complexity is $O(n \log n)$. The `min_rooms_required` function also includes a min-heap which requires $O(n \log n)$ total time for operations, and this is the most dominant cost, so the time complexity for both functions is $O(n \log n)$. In both functions, the heap holds at most $n$ entries, so the space complexity is $O(n)$.

---

## Trade-offs: Why HashMap + Doubly Linked List?

I have already answered the question of why we have used the combination of a hash map and a doubly linked list above. The hash map provides us with a searching speed of $O(1)$ and the doubly linked list provides us with deletion, insertion, and updation speeds of $O(1)$, completing our goal of getting $O(1)$ time complexity.
